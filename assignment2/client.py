    ...
    def send_request(self, command):
        ...
        try:
            ...
            try:
                ...
                if (response is None):
                    print(
                        "FAILURE: There is no leader Yet.Please wait for the election to complete.")
                    return None
                ...
                if response.success:
                    ...
                    if response.statusCode == 101:
                        # if failure to execute on state machine and sent to the correct leader
                        if (command.split()[0] == "GET"):
                            print(
                                "FAILURE: Incorrect Key Access. Empty string returned")
                        else:
                            print(
                                "FAILURE: Could not get majority. Could not commit the write request.")
                        return
                    else:
            ...
            print(
                "FAILURE: The Election is currently going on. Kindly retry after some time.")
            return None
            ...
            try:
                with grpc.insecure_channel(address) as channel:
                    ...
                    response = stub.ServeClient(
                        raft_pb2.ServeClientArgs(request="GET leader"))
                    # print(response)
                ...
                print(
                    f"FAILURE: Error finding leader on server {server_id}: {e}")
                continue
        ...
        if (self.leader_id):
            ...
            try:
                ...
                with grpc.insecure_channel(address) as channel:
                    ...
                    # print("here")
                    response = stub.ServeClient(
                        raft_pb2.ServeClientArgs(request=command, timeout=0.05))
                    # print("here 2")
            ...
            for server_id, address in self.servers.items():
                try:
                    with grpc.insecure_channel(address) as channel:
                        stub = raft_pb2_grpc.RaftServiceStub(channel)
                        response = stub.ServeClient(
                            raft_pb2.ServeClientArgs(request=command))
                        # print(response.leader_id,response.success,response.statusCode)
...
if __name__ == "__main__":
    ...
    servers = {0: '34.131.56.131:50050', 1: '34.131.128.179:50050',
               2: '34.131.143.80:50050', 3: '34.131.224.202:50050', 4: '34.131.51.75:50050'}
